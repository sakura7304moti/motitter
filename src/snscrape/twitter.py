...
class Tweet(base.Item):
	...
	username = base._DeprecatedProperty('username', lambda self: getattr(self.user, 'username', None), 'user.username')
	outlinks = base._DeprecatedProperty('outlinks', lambda self: [x.url for x in self.links] if self.links else [], 'links (url attribute)')
	outlinksss = base._DeprecatedProperty('outlinksss', lambda self: ' '.join(x.url for x in self.links) if self.links else '', 'links (url attribute)')
	tcooutlinks = base._DeprecatedProperty('tcooutlinks', lambda self: [x.tcourl for x in self.links] if self.links else [], 'links (tcourl attribute)')
	tcooutlinksss = base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(x.tcourl for x in self.links) if self.links else '', 'links (tcourl attribute)')
	content = base._DeprecatedProperty('content', lambda self: self.rawContent, 'rawContent')
...
class User(base.Item):
	...
	descriptionUrls = base._DeprecatedProperty('descriptionUrls', lambda self: self.descriptionLinks, 'descriptionLinks')
	linkUrl = base._DeprecatedProperty('linkUrl', lambda self: self.link.url if self.link else None, 'link.url')
	linkTcourl = base._DeprecatedProperty('linkTcourl', lambda self: self.link.tcourl if self.link else None, 'link.tcourl')
	description = base._DeprecatedProperty('description', lambda self: self.renderedDescription, 'renderedDescription')
...
class _TwitterAPIScraper(base.Scraper):
	...
	def _make_card(self, card, apiType, tweetId):
		...
		if cardName in ('summary', 'summary_large_image', 'app', 'direct_store_link_app'):
			...
			kwargs = utils.dict_map(bindingValues, {'end_datetime_utc': 'endDate', 'last_updated_datetime_utc': 'lastUpdateDate', 'duration_minutes': 'duration', 'counts_are_final': 'finalResults'})
			...
			return PlayerCard(**utils.dict_map(bindingValues, {'title': 'title', 'description': 'description', 'card_url': 'url', 'player_image_original': 'imageUrl', 'site': 'siteUser'}))
		elif cardName in ('promo_image_convo', 'promo_video_convo'):
			kwargs = utils.dict_map(bindingValues, {'thank_you_text': 'thankYouText', 'thank_you_url': 'thankYouUrl', 'thank_you_shortened_url': 'thankYouTcoUrl'})
			kwargs['actions'] = []
			...
			kwargs = utils.dict_map(bindingValues, {'event_id': 'id', 'event_title': 'title', 'event_category': 'category', 'event_subtitle': 'description'})
			...
			kwargs = utils.dict_map(bindingValues, {'newsletter_title': 'title', 'newsletter_description': 'description', 'newsletter_image_original': 'imageUrl', 'card_url': 'url', 'revue_account_id': 'revueAccountId', 'issue_count': 'issueCount'})
			...
			kwargs = utils.dict_map(bindingValues, {
			...
			kwargs = utils.dict_map(bindingValues, {'title': 'title', 'app_category': 'appCategory', 'player_owner_id': 'playerOwnerId', 'site': 'siteUser'})
			...
			return SpacesCard(**utils.dict_map(bindingValues, {'card_url': 'url', 'id': 'id'}))
			...
			if bindingValues['cta'] not in ctas:
				...
				return
			return MessageMeCard(**utils.dict_map(bindingValues, {'recipient': 'recipient', 'card_url': 'url'}), buttonText = ctas[bindingValues['cta']])
	...
	def _graphql_timeline_tweet_item_result_to_tweet(self, result, tweetId = None):
		if result['__typename'] == 'Tweet':
			...
			if tweetId is None:
				raise base.ScraperException('Cannot handle unavailable tweet without tweet ID')
					...
					if entry['content']['entryType'] == 'TimelineTimelineModule':
						for item in entry['content']['items']:
							if not item['entryId'].startswith('homeConversation-') or '-tweet-' not in item['entryId']:
								raise base.ScraperException(f'Unexpected home conversation entry ID: {item["entryId"]!r}')
...
class TwitterSearchScraper(_TwitterAPIScraper):
	...
	def __init__(self, query, *, cursor = None, mode = TwitterSearchScraperMode.LIVE, top = None, maxEmptyPages = 20, **kwargs):
		...
		if cursor is not None:
			warnings.warn('the `cursor` argument is deprecated', base.DeprecatedFeatureWarning, stacklevel = 2)
		self._cursor = cursor
		if top is not None:
			replacement = f'{__name__}.TwitterSearchScraperMode.' + ('TOP' if top else 'LIVE')
			warnings.warn(f'`top` argument is deprecated, use `mode = {replacement}` instead of `top = {bool(top)}`', base.DeprecatedFeatureWarning, stacklevel = 2)
	...
	def get_items(self):
		if self._isUserId:
			...
			if self.entity is None:
				raise base.ScraperException(f'Could not resolve user ID {self._user!r} to username')
...
class TwitterProfileScraper(TwitterUserScraper):
	...
	def get_items(self):
		if not self._isUserId:
			if self.entity is None:
				raise base.ScraperException(f'Could not resolve username {self._user!r} to ID')
...
class TwitterCashtagScraper(TwitterSearchScraper):
	...
	@classmethod
	def _cli_setup_parser(cls, subparser):
		subparser.add_argument('cashtag', type = base.nonempty_string('cashtag'), help = 'A Twitter cashtag (without $)')
