...
class Tweet(base.Item):
    ...
    username = base._DeprecatedProperty(
        "username", lambda self: getattr(self.user, "username", None), "user.username"
    )
    outlinks = base._DeprecatedProperty(
        "outlinks",
...
class UnifiedCard(Card):
    ...
    def __post_init__(self):
        if (self.components is not None) + (self.swipeableLayoutSlides is not None) + (
            self.collectionLayoutSlides is not None
        ) != 1:
            raise ValueError(
                "did not get exactly one of components, swipeableLayoutSlides, and collectionLayoutSlides"
            )
...
class _CLIGuestTokenManager(GuestTokenManager):
    ...
    def _read(self):
        ...
        validTokens = [
            token
            for token, t in o["tokens"].items()
            if t["setTime"] >= setTimeThreshold
            and t.get("blockedUntil", 0) < currentTime
        ]
        if not validTokens:
            return None
    ...
    def _write(self):
        with self._lock:
            ...
            o["tokens"] = {
                token: details
                for token, details in o["tokens"].items()
                if details["setTime"] >= setTimeThreshold
            }
...
class _TwitterTLSAdapter(base._HTTPSAdapter):
...
class _TwitterAPIScraper(base.Scraper):
    ...
    def _check_guest_token_response(self, r):
        if r.status_code != 200:
            return (
                False,
                ("non-200 response" if r.status_code != 404 else "blocked")
                + f" ({r.status_code})",
            )
        return True, None
        ...
        if self._guestTokenManager.token is None:
            ...
            if not self._guestTokenManager.token:
                ...
                if not o.get("guest_token"):
                    raise base.ScraperException("Unable to retrieve guest token")
    ...
    def _check_api_response(self, r):
        if r.status_code in (403, 404, 429):
            if (
                r.status_code == 429
                and r.headers.get("x-rate-limit-remaining", "") == "0"
                and "x-rate-limit-reset" in r.headers
            ):
                blockUntil = min(
                    int(r.headers["x-rate-limit-reset"]), int(time.time()) + 900
                )
            else:
                blockUntil = None
    ...
    def _get_api_data(self, endpoint, apiType, params, instructionsPath=None):
        ...
        if apiType is _TwitterAPIType.GRAPHQL and "errors" in obj:
            msg = "Twitter responded with an error: " + ", ".join(
                f'{e["name"]}: {e["message"]}' for e in obj["errors"]
            )
            instructions = obj
    ...
    def _iter_api_data(
        ...
        while True:
            ...
            obj = self._get_api_data(
                endpoint, apiType, reqParams, instructionsPath=instructionsPath
            )
            yield obj
            ...
            for instruction in instructions:
                ...
                for entry in entries:
                    ...
                    ) or entry["entryId"].startswith("cursor-showmorethreads-"):
    ...
    def _count_tweets_and_users(self, entries):
        return sum(
            ...
            or entry["entryId"].startswith("user-")
            for entry in entries
        )
            ...
            for entry in entries:
                if entry["entryId"].startswith("sq-I-t-") or entry[
                    ...
                    yield self._user_to_user(
                        obj["globalObjects"]["users"][
                            entry["content"]["item"]["content"]["user"]["id"]
                        ]
                    )
    ...
    def _make_tweet(
        ...
        **kwargs,
        ...
        kwargs[
            "url"
        ] = f'https://twitter.com/{getattr(user, "username", "i/web")}/status/{tweetId}'
    ...
    def _make_card(self, card, apiType, tweetId):
            ...
            for o in card["legacy"].get("user_refs_results", []):
                ...
                if userId in userRefs:
                    if userRefs[userId] != user:
                        _logger.warning(
                            f"Duplicate user {userId} with differing data in card on tweet {tweetId}"
                        )
                    continue
        ...
        for key, value in messyBindingValues:
            ...
            if value["type"] == "STRING":
                ...
                if bindingValues[key] is None:
                    _logger.warning(
                        f"User {userId} not found in user refs in card on tweet {tweetId}"
                    )
            else:
                _logger.warning(
        ...
        if cardName in (
                ...
                video=Video(
                    ...
                    variants=[
                        VideoVariant(
                            ...
                            contentType=bindingValues.get("player_stream_content_type"),
                            bitrate=None,
                        )
                    ],
                ),
            ...
            return SpacesCard(
                **utils.dict_map(bindingValues, {"card_url": "url", "id": "id"})
            )
            ...
            return MessageMeCard(
                **utils.dict_map(
                    bindingValues, {"recipient": "recipient", "card_url": "url"}
                ),
                ...
                if o["layout"]["type"] == "swipeable":
                    ...
                    kwargs["collectionLayoutSlides"] = [
                        UnifiedCardCollectionLayoutSlide(
                            detailsComponentKey=v[0], mediumComponentKey=v[1]
                        )
                        for v in o["layout"]["data"]["slides"]
                    ]
                else:
            ...
            # Consistency checks
            missingParts = set()
            if card.components and not all(
                k in card.componentObjects for k in card.components
            ):
                missingParts.add("components")
            if card.swipeableLayoutSlides and not all(
                s.mediumComponentKey in card.componentObjects
                and s.componentKey in card.componentObjects
                for s in card.swipeableLayoutSlides
            ):
                missingParts.add("components")
            if any(
                c.destinationKey not in card.destinations
                for c in card.componentObjects.values()
                if hasattr(c, "destinationKey")
            ):
                missingParts.add("destinations")
            if any(
                b.destinationKey not in card.destinations
                for c in card.componentObjects.values()
            ...
            mediaKeys.extend(
                d.mediumKey
                for d in card.destinations.values()
                if d.mediumKey is not None
            )
            mediaKeys.extend(
                a.iconMediumKey
                for l in (card.apps.values() if card.apps is not None else [])
                for a in l
            ...
            if any(
                c.appKey not in card.apps
                for c in card.componentObjects.values()
                if hasattr(c, "appKey")
            ):
                missingParts.add("apps")
            if any(
                d.appKey not in card.apps
                for d in card.destinations.values()
    ...
    def _make_tombstone(self, tweetId, info):
        if tweetId is None:
            raise base.ScraperException("Cannot create tombstone without tweet ID")
        if info and (text := info.get("richText", info["text"])):
            return Tombstone(
                ...
                textLinks=[
                    TextLink(
                        text=text["text"][x["fromIndex"] : x["toIndex"]],
                        url=x["ref"]["url"],
                        tcourl=None,
                        indices=(x["fromIndex"], x["toIndex"]),
                    )
        ...
        if result["__typename"] == "Tweet":
            ...
            if tweetId is None:
                raise base.ScraperException(
                    "Cannot handle unavailable tweet without tweet ID"
                )
            return TweetRef(id=tweetId)
        else:
            raise base.ScraperException(f'Unknown result type {result["__typename"]!r}')
        tweet = result["legacy"]
        user = self._graphql_user_results_to_user(
            result["core"]["user_results"], userId=int(result["legacy"]["user_id_str"])
        )
        kwargs = {}
        ...
        if "quoted_status_result" in result:
            if "result" not in result["quoted_status_result"]:
                ...
                kwargs[
                    ...
                    tweetId=int(tweet["quoted_status_id_str"]),
                )
            ...
            if result["quotedRefResult"]["result"]["__typename"] == "TweetTombstone":
                kwargs[
                    ...
                    result["quotedRefResult"]["result"],
                    tweetId=int(tweet["quoted_status_id_str"]),
                )
            else:
                qTweet = result["quotedRefResult"]["result"]
                if result["quotedRefResult"]["result"]["__typename"] not in (
                    "Tweet",
                    ...
                    result["quotedRefResult"]["result"]["__typename"]
                    == "TweetWithVisibilityResults"
                ):
                    qTweet = qTweet["tweet"]
            ...
            if tweet["quoted_status_id_str"] != tweet.get(
                "retweeted_status_result", {}
            ).get("result", {}).get("quoted_status_result", {}).get("result", {}).get(
                "rest_id"
            ):
    ...
    def _user_to_user(self, user, id_=None):
        ...
        if (
            "ext" in user
            and "highlightedLabel" in user["ext"]
            and (label := user["ext"]["highlightedLabel"]["r"]["ok"].get("label"))
        ):
    ...
    def _graphql_user_results_to_user_ref(self, obj, userId=None):
        ...
        if (
            "result" in obj
            and obj["result"]["__typename"] == "UserUnavailable"
            and "unavailable_message" in obj["result"]
        ):
            kwargs["text"] = obj["result"]["unavailable_message"]["text"]
            kwargs["textLinks"] = [
                TextLink(
                    text=kwargs["text"][x["fromIndex"] : x["toIndex"]],
                    url=x["ref"]["url"],
                    tcourl=None,
                    indices=(x["fromIndex"], x["toIndex"]),
                )
                for x in obj["result"]["unavailable_message"]["entities"]
            ]
    ...
    def _graphql_user_results_to_user(self, results, userId=None):
        if (
            ...
            or results["result"]["__typename"] == "UserUnavailable"
        ):
            return self._graphql_user_results_to_user_ref(results, userId)
        return self._user_to_user(
            results["result"]["legacy"],
            id_=userId if userId is not None else int(results["result"]["rest_id"]),
        )
...
class TwitterSearchScraper(_TwitterAPIScraper):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if top is not None:
            ...
            mode = (
                TwitterSearchScraperMode.TOP if top else TwitterSearchScraperMode.LIVE
            )
        self._mode = mode
    ...
    def get_items(self):
        ...
        if self._mode is TwitterSearchScraperMode.USER:
            raise base.ScraperException("User searches currently unsupported")
        ...
        for obj in self._iter_api_data(
            "https://twitter.com/i/api/graphql/7jT5GT59P8IFjgxwqnEdQw/SearchTimeline",
            ...
            yield from self._graphql_timeline_instructions_to_tweets(
                obj["data"]["search_by_raw_query"]["search_timeline"]["timeline"][
                    "instructions"
                ]
            )
    ...
    def _cli_setup_parser(cls, subparser):
        ...
        group.add_argument(
            ...
            action="store_true",
            default=False,
            help="Search users instead of tweets",
        )
        ...
        subparser.add_argument(
            "query", type=base.nonempty_string("query"), help="A Twitter search string"
        )
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def _get_entity(self):
        ...
        if not self._isUserId:
            ...
            fieldName = "userId"
            endpoint = (
                "https://twitter.com/i/api/graphql/1YAM811Q8Ry4XyPpJclURQ/UserByRestId"
            )
    ...
    def get_items(self):
        if self._isUserId:
            ...
            if self.entity is None:
                raise base.ScraperException(
                    f"Could not resolve user ID {self._user!r} to username"
                )
...
class TwitterProfileScraper(TwitterUserScraper):
    ...
    def get_items(self):
        ...
        gotPinned = False
        for obj in self._iter_api_data(
            "https://twitter.com/i/api/graphql/fn9oRltM1N4thkh5CVusPg/UserTweetsAndReplies",
            _TwitterAPIType.GRAPHQL,
            params,
            paginationParams,
            instructionsPath=[
            ...
            if not gotPinned:
                for instruction in instructions:
                    if instruction["type"] == "TimelinePinEntry":
                        gotPinned = True
                        tweetId = (
                            int(instruction["entry"]["entryId"][6:])
                            if instruction["entry"]["entryId"].startswith("tweet-")
                            else None
                        )
                        yield self._graphql_timeline_tweet_item_result_to_tweet(
...
class TwitterHashtagScraper(TwitterSearchScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "hashtag",
            type=base.nonempty_string("hashtag"),
...
class TwitterCashtagScraper(TwitterSearchScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "cashtag",
            type=base.nonempty_string("cashtag"),
...
class TwitterTweetScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            ...
            if not obj["data"]:
                return
            for instruction in obj["data"]["threaded_conversation_with_injections_v2"][
                "instructions"
            ]:
                if instruction["type"] != "TimelineAddEntries":
                    continue
            ...
            while queue:
                ...
                del (
                    thisPagParams["variables"]["cursor"],
                    thisPagParams["variables"]["referrer"],
                )
                for obj in self._iter_api_data(
                    url,
                    _TwitterAPIType.GRAPHQL,
...
class TwitterListPostsScraper(TwitterSearchScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "list",
            type=base.nonempty_string("list"),
...
class TwitterCommunityScraper(_TwitterAPIScraper):
    ...
    def __init__(self, communityId, **kwargs):
        ...
        super().__init__(
            f"https://twitter.com/i/communities/{self._communityId}", **kwargs
        )
    ...
    def _get_entity(self):
        ...
        obj = self._get_api_data(
            "https://twitter.com/i/api/graphql/bC3Saf4niY6YuzJWV2oUGg/CommunitiesFetchOneQuery",
            _TwitterAPIType.GRAPHQL,
            params=params,
            instructionsPath=["data", "communityResults"],
        )
        if not obj["data"] or "result" not in obj["data"]["communityResults"]:
            _logger.warning("Empty response")
            return None
        if (
            obj["data"]["communityResults"]["result"]["__typename"]
        ...
        return Community(
            ...
            created=datetime.datetime.fromtimestamp(
                community["created_at"] / 1000, tz=datetime.timezone.utc
            ),
            ...
            membersFacepile=[
                self._graphql_user_results_to_user(m)
                for m in community["members_facepile_results"]
            ],
            ...
            bannerUrl=community.get(
                "custom_banner_media", community["default_banner_media"]
    ...
    def get_items(self):
        ...
        for obj in self._iter_api_data(
            "https://twitter.com/i/api/graphql/9nnDM-yum8Te--T2REfgkg/CommunityTweetsTimeline",
            _TwitterAPIType.GRAPHQL,
            params,
            paginationParams,
            instructionsPath=[
            ...
            if (
                ...
                break
            yield from self._graphql_timeline_instructions_to_tweets(
                obj["data"]["communityResults"]["result"]["community_timeline"][
                    "timeline"
                ]["instructions"]
            )
...
__getattr__, __dir__ = base._module_deprecation_helper(__all__, DescriptionURL=TextLink)
